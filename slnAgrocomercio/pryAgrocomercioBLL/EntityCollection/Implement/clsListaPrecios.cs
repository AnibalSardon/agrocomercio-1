...
#region FUNCIONES DE CONSULTA
        ...
        public DataTable GetListaPreciosLotes(int pArtCod)
        {
            ...
            var lstPreciosTmp = this.GetAll().Where(Pre => Pre.ArtCod == pArtCod)
                    .Select(Pre => new
                    {
                        Pre.LprCod,
                        Pre.ArtCod,
        ...
        {
            var result = (double)this.Find(Pre => Pre.ArtCod == ArtCod && Pre.LprPrecio > 0).
                Select(Pre => (Pre.LprPrecio - (Pre.LprPrecio * Pre.LprDscto / 100) + 
